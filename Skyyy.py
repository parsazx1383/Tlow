...
DBName = "amyeyenn_SEX" # Database Name
api_channel = "SHAH_SELF"  # یا از تنظیمات بخوانید
DBUser = "amyeyenn_SEX" # Database User
DBPass = "Zxcvbnm1111" # Database Password
HelperDBName = "amyeyenn_SEX1" # Helper Database Name
HelperDBUser = "amyeyenn_SEX1" # Helper Database User
...
def get_data_direct(query, params=None):
    """دریافت داده مستقیم از دیتابیس - بدون pool"""
    try:
        ...
        print(f"❌ get_data_direct error: {e}")
        return None

def update_data_direct(query, params=None):
    ...
    try:
        ...
        connection.close()
        print(f"✅ update_data_direct: {affected} rows affected")
        return affected
    except Exception as e:
        print(f"❌ update_data_direct error: {e}")
        return 0
...
def get_data(query, use_cache=True, cache_key=None, params=None):
    """تابع بهینه‌شده get_data"""
    # تمیزکردن کش‌های قدیمی
...
def get_user_info(user_id):
    """خیلی ساده کاربر را بگیر یا بساز"""
    try:
        # اول مستقیم چک کن
        query = f"SELECT * FROM user WHERE id = {user_id}"
        result = get_data_direct(query)
        
        if result:
            print(f"✅ User {user_id} found in DB")
            return result
        
        # اگر نبود، بساز
        print(f"⚠️ User {user_id} not found, creating...")
        create_query = f"""
        INSERT INTO user (id, step, expir, phone, account, self) 
        VALUES ({user_id}, 'none', 0, NULL, 'unverified', 'inactive')
        """
        
        if update_data_direct(create_query) > 0:
            # دوباره بگیر
            new_result = get_data_direct(query)
            if new_result:
        ...
        print(f"❌ get_user_info_simple error: {e}")
        return {"id": user_id, "step": "none", "expir": 0}
...
def checker(func):
    ...
    async def wrapper(c, m, *args, **kwargs):
        ...
        # بررسی عضویت در کانال (فقط برای کاربران عادی)
        if chat_id != Admin:
            try:
                ...
                if not is_member:
                    ...
                    await app.send_message(
                        chat_id,
                        "**• لطفاً ابتدا در کانال ما عضو شوید:**",
                        reply_markup=InlineKeyboardMarkup([
                            [InlineKeyboardButton(text=channel_name, url=f"https://t.me/{Channel_ID}")],
                ...
                if chat_id == Admin:
                    await app.send_message(Admin, "**خطا در بررسی کانال**")
                return
...
def get_main_keyboard(user_id, expir, is_admin=False, has_self_folder=False, current_lang="fa"):
    ...
    # ردیف 3: انقضا
    keyboard.append([InlineKeyboardButton(text=f"انقضا: {expir} روز", callback_data="ExpiryStatus")])
...
@app.on_message(filters.private & filters.command(["start", "restart"]))
...
به دستور ( {call.from_user.id} ) رد شد.**""")
        ...
        try:
            ...
            # 2. دریافت اطلاعات کاربر با کوئری مستقیم و ساده
            query = "SELECT expir, step FROM user WHERE id = %s"
            user_data = get_data(query, params=[call.from_user.id], use_cache=False)
            
            if not user_data:
                # اگر باز هم خطا داشت، مقادیر پیش‌فرض
                expir = 0
                step = "none"
                print(f"Warning: Could not get user data for {call.from_user.id}")
            else:
                expir = user_data.get("expir", 0)
                step = user_data.get("step", "none")
            ...
            # 3. مقادیر دیگر
            is_admin = (call.from_user.id == Admin)
            ...
            # 5. آپدیت step
            update_data("UPDATE user SET step = 'none' WHERE id = %s", params=[call.from_user.id])
            
            # 6. پاک کردن temp_Client اگر وجود دارد
            async with lock:
                if call.from_user.id in temp_Client:
                    del temp_Client[call.from_user.id]
            
            # 7. ارسال پیام
            await app.edit_message_text(
                call.message.chat.id,
                call.message.id,
                "**به منوی اصلی بازگشتید.** ✅\n\nلطفا اگر سوالی دارید از بخش پشتیبانی استفاده کنید.",
                reply_markup=keyboard
            )
            
            print(f"Back button successful for user {call.from_user.id}")
            
        except Exception as e:
            print(f"ERROR in Back callback for {call.from_user.id}: {str(e)}")
            # پیام خطا به کاربر
            await app.answer_callback_query(
                call.id,
                text="خطا در بازگشت به منوی اصلی. لطفا /start را بزنید.",
                show_alert=True
            )
...
async def test_database(c, m):
    ...
    try:
        ...
        # تست INSERT
        with connection.cursor() as cursor:
            cursor.execute(f"INSERT INTO user(id, step) VALUES({m.chat.id}, 'test')")
            connection.commit()
        
        # تست SELECT
        with connection.cursor() as cursor:
            cursor.execute(f"SELECT * FROM user WHERE id = {m.chat.id}")
            result = cursor.fetchone()
            
        connection.close()
        
        if result:
            await app.send_message(m.chat.id, f"✅ دیتابیس کار می‌کند\nآیدی: {result['id']}\nStep: {result.get('step')}")
        else:
            await app.send_message(m.chat.id, "❌ مشکل در دیتابیس")
            
    except Exception as e:
        await app.send_message(m.chat.id, f"❌ خطای دیتابیس: {str(e)[:100]}")
