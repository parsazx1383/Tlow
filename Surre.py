...
def get_user_info(user_id):
    """دریافت کامل و درست اطلاعات کاربر"""
    try:
        # ابتدا بررسی ساده از دیتابیس
        query = "SELECT * FROM user WHERE id = %s"
        result = get_data(query, params=[user_id])
        
        # اگر کاربر وجود نداشت، ایجاد کن
        if not result:
            print(f"📝 Creating new user: {user_id}")
            create_query = """
            INSERT INTO user(id, step, phone, expir, api_id, api_hash, account, self, pid, last_language_change) 
            VALUES(%s, 'none', NULL, 0, NULL, NULL, 'unverified', 'inactive', NULL, NULL)
            """
            update_data(create_query, params=[user_id])
            
            # دوباره دریافت کن
            result = get_data(query, params=[user_id])
            
            # اگر باز هم نشد، حداقل اطلاعات را برگردان
            if not result:
                return {
                    "id": user_id,
                    "step": "none",
        ...
        # اصلاح مقادیر None
        if user_info["step"] is None:
            user_info["step"] = "none"
        if user_info["expir"] is None:
            user_info["expir"] = 0
            
        print(f"✅ User info for {user_id}: step={user_info['step']}, expir={user_info['expir']}")
        
        return user_info
        
    except Exception as e:
        print(f"❌ ERROR in get_user_info for {user_id}: {str(e)[:100]}")
        # بازگشت حداقلی در صورت خطا
        return {
            "id": user_id,
            "step": "none",
            "phone": None,
...
async def message_handler(c, m):
    ...
    print(f"DEBUG: User {chat_id}, Step: {user.get('step')}, Text: {text[:50]}")
