...
def get_user_info(user_id):
    """دریافت کاربر - بسیار ساده و مطمئن"""
    try:
        # کوئری مستقیم و ساده
        query = f"SELECT * FROM user WHERE id = {user_id}"
        result = get_data(query, use_cache=False)  # کش را غیرفعال کن
        
        if result:
            print(f"✅ Found user {user_id}, step: {result.get('step', 'none')}")
            return result
        else:
            ...
            VALUES({user_id}, 'none', 0)
            """
            update_data(create_query)
            
            # فوری دریافت کن
            new_query = f"SELECT * FROM user WHERE id = {user_id}"
            new_result = get_data(new_query, use_cache=False)
...
async def message_handler(c, m):
    ...
    result = get_data(query, use_cache=False)
    
    if result:
        current_step = result.get("step", "none")
        print(f"🔍 User {chat_id}, Current step from DB: {current_step}")
    else:
        current_step = "none"
        print(f"⚠️ User {chat_id} not found in message_handler")
