...
def get_data_direct(query, params=None):
    ...
    try:
        ...
        if result:
            print(f"✅ get_data_direct success: {query[:50]}...")
        else:
            print(f"⚠️ get_data_direct no result: {query[:50]}...")
...
def get_user_info(user_id):
    ...
    try:
        ...
        result = get_data_direct(f"SELECT * FROM user WHERE id = {user_id}")
        ...
        if update_data_direct(insert_query) > 0:
            # دوباره بگیر
            new_result = get_data_direct(f"SELECT * FROM user WHERE id = {user_id}")
            if new_result:
