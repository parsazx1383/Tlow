...
به دستور ( {call.from_user.id} ) رد شد.**""")
        ...
        # بررسی ادمین بودن
        is_admin = (call.from_user.id == Admin or 
                    helper_getdata(f"SELECT * FROM adminlist WHERE id = '{call.from_user.id}' LIMIT 1") is not None)
        
        # بررسی وجود پوشه سلف
        has_self_folder = os.path.isdir(f"selfs/self-{call.from_user.id}")
        
        # دریافت زبان فعلی
        current_lang = get_current_language(call.from_user.id)
        
        # ایجاد کیبورد با پارامترهای صحیح
        keyboard = get_main_keyboard(
