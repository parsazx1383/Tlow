...
db_pool = PooledDB(
    ...
    autocommit=True,  # فعال کردن autocommit
    connect_timeout=5,
    read_timeout=20,
    write_timeout=20,
    threadlocal=True  # بهبود performance در multithreading
)
...
def checker(func):
    ...
    async def wrapper(c, m, *args, **kwargs):
        ...
        if chat_id != Admin:
            try:
                is_member = await check_channel_membership_cached(chat_id)
                if not is_member:
                    try:
...
def get_setting(key, default=None):
    ...
    with _cache_lock:
        if cache_key in _settings_cache:
            value, timestamp = _settings_cache[cache_key]
            if time.time() - timestamp < 300:  # 5 دقیقه کش برای تنظیمات
                return value
    
    # دریافت از دیتابیس
    result = get_data("SELECT setting_value FROM settings WHERE setting_key = %s", params=[key])
    value = result["setting_value"] if result else default
    
    with _cache_lock:
        _settings_cache[cache_key] = (value, time.time())
...
@lru_cache(maxsize=1000)
def get_main_keyboard_fast(user_id, expir, is_admin=False, has_self_folder=False, current_lang="fa"):
...
def get_main_keyboard(user_id):
    """تابع اصلی دریافت کیبورد با کش‌گذاری کامل"""
    # دریافت اطلاعات کاربر از کش
    user_data = get_user_all(user_id)
    if not user_data:
        user_data = {"expir": 0}
    ...
    # بررسی ادمین
    is_admin = user_id == Admin or helper_getdata(f"SELECT * FROM adminlist WHERE id = '{user_id}'") is not None
    
    # بررسی وجود پوشه سلف
    has_self_folder = check_file_cached(f"selfs/self-{user_id}")
    
    # دریافت زبان فعلی
    current_lang = get_current_language_cached(user_id)
    
    return get_main_keyboard_fast(user_id, expir, is_admin, has_self_folder, current_lang)
...
async def process_callback_background(client, callback_query):
    """پردازش callback در background"""
    try:
        ...
        if data == "Back":
            await handle_back_callback(client, callback_query)
        elif data == "Price":
            await handle_price_callback(client, callback_query)
        elif data == "Support":
            await handle_support_callback(client, callback_query)
        elif data.startswith("VerifyPayment-"):
            await handle_verify_payment_callback(client, callback_query)
        # ... سایر callbackها
...
async def callback_handler(c, call):
    ...
    asyncio.create_task(process_callback_background(c, call))
...
• LRU Settings Cache: {get_setting_cached.cache_info()}
        """
